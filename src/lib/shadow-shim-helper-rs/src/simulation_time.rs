...
impl core::convert::TryFrom<linux_api::time::kernel_timespec> for SimulationTime {
    type Error = ();

    fn try_from(value: linux_api::time::kernel_timespec) -> Result<Self, Self::Error> {
        if value.tv_sec < 0 || value.tv_nsec < 0 || value.tv_nsec > 999_999_999 {
            return Err(());
        }
        let secs = Duration::from_secs(value.tv_sec.try_into().unwrap());
...
impl core::convert::TryFrom<SimulationTime> for linux_api::time::kernel_timespec {
    type Error = ();
    ...
    fn try_from(value: SimulationTime) -> Result<Self, Self::Error> {
        ...
        let tv_sec = value.as_secs().try_into().map_err(|_| ())?;
        let tv_nsec = value.subsec_nanos().into();
        Ok(linux_api::time::kernel_timespec { tv_sec, tv_nsec })
    }
}
...
pub const SIMTIME_MAX: CSimulationTime = 16673476130709551614u64;
const _: () =
